// To run: npx jest fretboardEngine/tests/chordGenerator.test.tsx
import fs from "fs";
...
describe("ChordGenerator", () => {
  ...
  test("generate E9 chords and check structure", () => {
    ...
    expect("m6" in chords).toBeTruthy();
    expect("M6/9" in chords).toBeTruthy();
    // suspended
    ...
    expect("7sus4" in chords).toBeTruthy();
    expect("11" in chords).toBeTruthy();
    expect("13" in chords).toBeTruthy();
    expect("7b5" in chords).toBeTruthy();
    expect("7#5" in chords).toBeTruthy();
    expect("7#11" in chords).toBeTruthy();
    ...
    const expectedVoicing = mChord.voicings.find(
      (voicing: any) =>
        JSON.stringify(voicing.notes) === JSON.stringify([7, 7, 7, "x", 7, 7, 7, 7, 7, "x"]) &&
        JSON.stringify(voicing.intervals) === JSON.stringify(["3", "3", "5", "x", "1", "3", "5", "1", "3", "x"]) &&
        JSON.stringify(voicing.pedals) === JSON.stringify(["A", "B", "D"]),
    );
